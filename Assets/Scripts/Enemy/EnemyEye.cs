    ...
    public class EnemyEye : MonoBehaviour
    {
        ...
        public bool IsPlayerInSight()
        {
            ...
            if (angle >= 360 - angleRange || angle <= angleRange)
            {
                ...
                RaycastHit2D raycastHit = Physics2D.Raycast(pos,
                    (PlayerController.Singleton.transform.position - pos).normalized, sightDistance, raycastLayer);
        ...
        private void OnDrawGizmos()
        {
            ...
            Gizmos.DrawLine(position,
                new Vector2(position.x + sightDistance * Mathf.Sin((angleRange - eulerAngles.z) * Mathf.Deg2Rad),
                    position.y + sightDistance * Mathf.Cos((angleRange - eulerAngles.z) * Mathf.Deg2Rad)));
            Gizmos.DrawLine(position,
                new Vector2(position.x + sightDistance * Mathf.Sin(-(angleRange + eulerAngles.z) * Mathf.Deg2Rad),
                    position.y + sightDistance * Mathf.Cos(-(angleRange + eulerAngles.z) * Mathf.Deg2Rad)));
        }
