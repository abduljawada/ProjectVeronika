...
namespace Pathfinding
{
    ...
    public class AstarPathEditor : Editor
    {
        ...
        {
            get { return script.data.graphTypes; }
        }
        ...
        public void OnEnable()
        {
            ...
            if ((!Application.isPlaying &&
                 (script.data == null || script.data.graphs == null || script.data.graphs.Length == 0)) ||
        ...
        static void EnableJs()
        {
            ...
            {
                string error =
                    "Could not enable Js support. AstarPathfindingProject folder did not exist in the default location.\n" +
        ...
        static void DisableJs()
        {
            ...
            {
                Debug.LogWarning("Could not disable JS - Could not find directory '" + Application.dataPath +
            ...
            {
                string error =
                    AssetDatabase.MoveAsset("Assets/AstarPathfindingEditor/Editor", scriptsFolder + "/Editor");
                if (error != "")
                {
                    Debug.LogError("Couldn't disable Js - " + error);
                }
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (!LoadStyles())
            {
                EditorGUILayout.HelpBox("The GUISkin 'AstarEditorSkin.guiskin' in the folder " +
                                        EditorResourceHelper.editorAssets +
                                        "/ was not found or some custom styles in it does not exist.\n" +
                                        ...
                                        "Skin loading is done in the AstarPathEditor.cs --> LoadStyles method",
                    MessageType.Error);
                return;
            }
            ...
            if (UnityEditor.EditorSettings.enterPlayModeOptionsEnabled &&
                (UnityEditor.EditorSettings.enterPlayModeOptions & EnterPlayModeOptions.DisableSceneReload) != 0)
            {
                EditorGUILayout.HelpBox(
                    "The enter play mode option 'Scene Reload' must be enabled. This package does not support it being disabled. Disabling domain reload is supported however.",
                    MessageType.Error);
            ...
            if (GUILayout.Button(new GUIContent("Scan",
                    "Recalculate all graphs. Shortcut cmd+alt+s ( ctrl+alt+s on windows )")))
            {
                MenuScan();
            }
        ...
        void DrawAboutArea()
        {
            ...
            if (aboutArea.BeginFade())
            {
                ...
                if (GUILayout.Button(new GUIContent("Documentation",
                        "Open the documentation for the A* Pathfinding Project")))
                {
                    Application.OpenURL(AstarUpdateChecker.GetURL("documentation"));
                }

                if (GUILayout.Button(new GUIContent("Project Homepage",
                        "Open the homepage for the A* Pathfinding Project")))
                {
        ...
        void DrawGraphHeader(GraphEditor graphEditor)
        {
            ...
            if (script.prioritizeGraphs)
            {
                ...
                var moveDown = GUILayout.Button(new GUIContent("Down", "Decrease the graph priority"),
                    GUILayout.Width(40));
                ...
                if (moveUp || moveDown)
                {
                    ...
                    else
                    {
                        ...
                        next = index + 1;
                        for (; next < script.graphs.Length; next++)
                            if (script.graphs[next] != null)
                                break;
                    }
            ...
            graph.drawGizmos = GUILayout.Toggle(graph.drawGizmos, new GUIContent("Draw Gizmos", "Draw Gizmos"),
        ...
        void DrawGraphInfoArea(GraphEditor graphEditor)
        {
            ...
            if (graphEditor.infoFadeArea.BeginFade())
            {
                ...
                graphNodeCounts = graphNodeCounts ??
                                  new Dictionary<NavGraph, KeyValuePair<float, KeyValuePair<int, int>>>();

                if (!graphNodeCounts.TryGetValue(graphEditor.target, out pair) ||
                    (Time.realtimeSinceStartup - pair.Key) > 2)
                {
                    ...
                    pair = new KeyValuePair<float, KeyValuePair<int, int>>(Time.realtimeSinceStartup,
                        new KeyValuePair<int, int>(total, numWalkable));
                ...
                if (total == 0)
                    EditorGUILayout.HelpBox("The number of nodes in the graph is zero. The graph might not be scanned",
                        MessageType.Info);
        ...
        void DrawSceneGUISettings()
        {
            ...
            GUILayout.BeginArea(
                new Rect(Camera.current.pixelWidth - width, Camera.current.pixelHeight - height, width - margin,
                    height - margin), "Graph Display", astarSkin.FindStyle("SceneBoxDark"));
            ...
            if (serializationSettingsArea.BeginFade())
            {
                script.data.cacheStartup = EditorGUILayout.Toggle(
                    new GUIContent("Cache startup",
                        "If enabled, will cache the graphs so they don't have to be scanned at startup"),
                ...
                {
                    EditorGUILayout.HelpBox(
                        "All graph settings will be replaced with the ones from the cache when the game starts",
                        MessageType.Info);
                }
                ...
                if (GUILayout.Button("Generate cache"))
                {
                    ...
                    if (EditorUtility.DisplayDialog("Scan before generating cache?",
                            "Do you want to scan the graphs before saving the cache.\n" +
                            "If the graphs have not been scanned then the cache may not contain node data and then the graphs will have to be scanned at startup anyway.",
                            "Scan", "Don't scan"))
                    {
                        MenuScan();
                    }
                ...
                {
                    if (EditorUtility.DisplayDialog("Are you sure you want to load from cache?",
                            "Are you sure you want to load graphs from the cache, this will replace your current graphs?",
                            "Yes", "Cancel"))
                    {
                ...
                {
                    EditorGUILayout.HelpBox(
                        "Storing the cached starup data on the AstarPath object has been deprecated. It is now stored " +
                ...
                if (GUILayout.Button("Save to file"))
                {
                    ...
                    if (path != "")
                    {
                        ...
                        if (EditorUtility.DisplayDialog("Include node data?",
                                "Do you want to include node data in the save file. " +
                                "If node data is included the graph can be restored completely without having to scan it first.",
                                "Include node data", "Only settings"))
                        {
                            serializationSettings.nodes = true;
                        }

                        if (serializationSettings.nodes && EditorUtility.DisplayDialog("Scan before saving?",
                                "Do you want to scan the graphs before saving? " +
                                "\nNot scanning can cause node data to be omitted from the file if the graph is not yet scanned.",
                                "Scan", "Don't scan"))
                        {
                            MenuScan();
                        }
        ...
        void DrawPathfindingSettings()
        {
            ...
            script.threadCount = (ThreadCount)EditorGUILayout.EnumPopup(new GUIContent("Thread Count",
                "Number of threads to run the pathfinding in (if any). More threads " +
            ...
            if (threads > 0)
                EditorGUILayout.HelpBox("Using " + threads + " thread(s)" +
                                        (script.threadCount < 0 ? " on your machine" : "") + ".\n" +
                                        "The free version of the A* Pathfinding Project is limited to at most one thread.",
                    MessageType.None);
            else
                EditorGUILayout.HelpBox(
            ...
            if (script.data.FindGraphWhichInheritsFrom(typeof(NavmeshBase)) != null)
            {
                script.navmeshUpdates.updateInterval = EditorGUILayout.FloatField(
                    new GUIContent("Navmesh Cutting Update Interval (s)",
                        "How often to check if any navmesh cut has changed."), script.navmeshUpdates.updateInterval);
            }

            script.prioritizeGraphs = EditorGUILayout.Toggle(new GUIContent("Prioritize Graphs",
                    "Normally, the system will search for the closest node in all graphs and choose the closest one" +
            ...
            {
                EditorGUI.indentLevel++;
                script.prioritizeGraphsLimit =
                    EditorGUILayout.FloatField("Priority Limit", script.prioritizeGraphsLimit);
                EditorGUI.indentLevel--;
            }

            script.fullGetNearestSearch = EditorGUILayout.Toggle(new GUIContent("Full Get Nearest Node Search",
                    "Forces more accurate searches on all graphs. " +
                    "Normally only the closest graph in the initial fast check will perform additional searches, " +
                    "if this is toggled, all graphs will do additional searches. Slower, but more accurate"),
                script.fullGetNearestSearch);
#endif
            script.scanOnStartup = EditorGUILayout.Toggle(
                new GUIContent("Scan on Awake",
                    "Scan all graphs on Awake. If this is false, you must call AstarPath.active.Scan () yourself. Useful if you want to make changes to the graphs with code."),
        ...
        void DrawEditorSettings()
        {
            ...
            if (editorSettingsArea.BeginFade())
            {
                ...
                if (IsJsEnabled())
                {
                    if (GUILayout.Button(new GUIContent("Disable Js Support",
                            "Revert to only enable pathfinding calls from C#")))
                    {
                        DisableJs();
                    }
                }
                ...
                {
                    if (GUILayout.Button(new GUIContent("Enable Js Support",
                            "Folders can be restructured to enable pathfinding calls from Js instead of just from C#")))
                    {
                        EnableJs();
                    }
                }
        ...
        void DrawDebugSettings()
        {
            ...
            if (script.debugMode == GraphDebugMode.SolidColor)
            {
                ...
                script.colorSettings._SolidColor = EditorGUILayout.ColorField(
                    new GUIContent("Color", "Color used for the graph when 'Graph Coloring'='Solid Color'"),
            ...
            {
                EditorGUILayout.HelpBox(
                    "Show Search Tree is enabled, you may see rendering glitches in the graph rendering" +
        ...
        void DrawColorSettings()
        {
            ...
            if (colorSettingsArea.BeginFade())
            {
                ...
                colors._SolidColor = EditorGUILayout.ColorField(
                    new GUIContent("Solid Color", "Color used for the graph when 'Graph Coloring'='Solid Color'"),
                    colors._SolidColor);
                ...
                colors._ConnectionLowLerp =
                    EditorGUILayout.ColorField("Connection Gradient (low)", colors._ConnectionLowLerp);
                colors._ConnectionHighLerp =
                    EditorGUILayout.ColorField("Connection Gradient (high)", colors._ConnectionHighLerp);
        ...
        {
            if (forceRebuild || graphEditors == null || script.graphs == null ||
                script.graphs.Length != graphEditors.Length)
            {
            ...
            else
            {
                for (int i = 0; i < script.graphs.Length; i++)
                {
                    ...
                    if (graphEditors[i] == null || graphEditorTypes[script.graphs[i].GetType().Name].editorType !=
                        graphEditors[i].GetType())
                    {
        ...
        GraphEditor CreateGraphEditor(NavGraph graph)
        {
            ...
            {
                Debug.LogError("Couldn't find an editor for the graph type '" + graphType + "' There are " +
                               graphEditorTypes.Count + " available graph editors");
        ...
        void DeserializeGraphs(byte[] bytes)
        {
            try
            {
                ...
                for (int i = 0; i < graphEditors.Length; i++)
                {
                    ...
                    if (data != null)
                        Pathfinding.Serialization.TinyJsonDeserializer.Deserialize(data, graphEditors[i].GetType(),
                            graphEditors[i], script.gameObject);
                }
            }
            catch (System.Exception e)
            {
        ...
        public static void MenuScan()
        {
            ...
            if (!Application.isPlaying &&
                (AstarPath.active.data.graphs == null || AstarPath.active.data.graphTypes == null))
            {
            ...
            catch (System.Exception e)
            {
                ...
                EditorUtility.DisplayDialog("Error Generating Graphs",
                    "There was an error when generating graphs, check the console for more info", "Ok");
                throw e;
            }
            finally
            {
