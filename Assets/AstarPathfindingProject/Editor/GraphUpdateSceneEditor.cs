...
namespace Pathfinding
{
    ...
    public class GraphUpdateSceneEditor : EditorBase
    {
        ...
        protected override void Inspector()
        {
            ...
            if (!points.hasMultipleDifferentValues && points.arraySize == 0)
            {
                ...
                else
                {
                    EditorGUILayout.HelpBox(
                        "No points and no collider or renderer attached, will not affect anything\nPoints can be added using the transform tool and holding shift",
                        MessageType.Warning);
                }
            }
            ...
            PropertyField("updateErosion", null,
                "Recalculate erosion for grid graphs.\nSee online documentation for more info");
            ...
            {
                EditorGUILayout.HelpBox(
                    "Legacy mode is enabled because you have upgraded from an earlier version of the A* Pathfinding Project. " +
                    "Disabling legacy mode is recommended but you may have to tweak the point locations or object rotation in some cases",
                    MessageType.Warning);
            ...
            if (scripts.Length == 1 && scripts[0].points.Length >= 3)
            {
                ...
                {
                    EditorGUILayout.HelpBox(
                        "The bounding box is very thin. Your shape might be oriented incorrectly. The shape will be projected down on the XZ plane in local space. Rotate this object " +
                        ...
                        "this object should have the rotation (-90,0,0). You will need to recreate your shape after rotating this object.",
                        MessageType.Warning);
                }
            }
        ...
        {
            if (PropertyField("updatePhysics", "Update Physics",
                    "Perform similar calculations on the nodes as during scan.\n" +
        ...
        void DrawPenaltyField()
        {
            ...
            {
                EditorGUILayout.HelpBox(
                    "Be careful when lowering the penalty. Negative penalties are not supported and will instead underflow and get really high.\n" +
                    "You can set an initial penalty on graphs (see their settings) and then lower them like this to get regions which are easier to traverse.",
                    MessageType.Warning);
            }
        }
        ...
        public void OnSceneGUI()
        {
            ...
            for (int i = 0; i < points.Count; i++)
            {
                if (i == selectedPoint && Tools.current == Tool.Move)
                {
                    ...
                    SphereCap(-i - 1, points[i], Quaternion.identity,
                        HandleUtility.GetHandleSize(points[i]) * pointGizmosRadius * 2);
                ...
                else
                {
                    ...
                    SphereCap(-i - 1, points[i], Quaternion.identity,
                        HandleUtility.GetHandleSize(points[i]) * pointGizmosRadius);
                }
            }
                ...
                if (((int)Event.current.modifiers & (int)EventModifiers.Alt) != 0)
                {
                    ...
                    else if (points.Count > 0)
                    {
                        ...
                        if (index >= 0 && index < points.Count)
                        {
                            ...
                            SphereCap(0, points[index], Quaternion.identity,
                                HandleUtility.GetHandleSize(points[index]) * 2f * pointGizmosRadius);
                        }
                    }
                }
                else
                {
                    ...
                    if (didHit)
                    {
                        ...
                        else if (points.Count > 0)
                        {
                            ...
                            Handles.DrawDottedLine(points[(insertionIndex - 1 + points.Count) % points.Count], rayhit,
                                8);
                            Handles.DrawDottedLine(points[insertionIndex % points.Count], rayhit, 8);
                            SphereCap(0, rayhit, Quaternion.identity,
                                HandleUtility.GetHandleSize(rayhit) * pointGizmosRadius);
