...
namespace Pathfinding
{
    ...
    {
        [JsonMember] public bool locked = true;

        [JsonMember] public bool showExtra;
        ...
        [JsonMember] public GridPivot pivot;

        [JsonMember] public bool collisionPreviewOpen = true;
        ...
        public static Vector3 RoundVector3(Vector3 v)
        {
            ...
            if (Mathf.Abs(Multiplier * v.x - Mathf.Round(Multiplier * v.x)) < 0.001f)
                v.x = Mathf.Round(Multiplier * v.x) / Multiplier;
            if (Mathf.Abs(Multiplier * v.y - Mathf.Round(Multiplier * v.y)) < 0.001f)
                v.y = Mathf.Round(Multiplier * v.y) / Multiplier;
            if (Mathf.Abs(Multiplier * v.z - Mathf.Round(Multiplier * v.z)) < 0.001f)
                v.z = Mathf.Round(Multiplier * v.z) / Multiplier;
            return v;
        }
        ...
        {
            return Mathf.Approximately(graph.isometricAngle, GridGraph.StandardIsometricAngle) &&
                   graph.neighbours == NumNeighbours.Six && graph.uniformEdgeCosts;
        }
        ...
        GUIContent[] hexagonSizeContents =
        {
            ...
            new GUIContent("Node Size",
                "Raw node size value, this doesn't correspond to anything particular on the hexagon."),
        };
        ...
        void DrawFirstSection(GridGraph graph)
        {
            ...
            if (graph.inspectorGridMode == InspectorGridMode.Hexagonal)
            {
                ...
                graph.inspectorHexagonSizeMode =
                    (InspectorGridHexagonNodeSize)EditorGUILayout.EnumPopup(new GUIContent("Hexagon Dimension"),
                        graph.inspectorHexagonSizeMode);
                float hexagonSize =
                    GridGraph.ConvertNodeSizeToHexagonSize(graph.inspectorHexagonSizeMode, graph.nodeSize);
                hexagonSize = (float)System.Math.Round(hexagonSize, 5);
                newNodeSize = GridGraph.ConvertHexagonSizeToNodeSize(graph.inspectorHexagonSizeMode,
                    EditorGUILayout.FloatField(hexagonSizeContents[(int)graph.inspectorHexagonSizeMode], hexagonSize));
                EditorGUILayout.EndVertical();
                if (graph.inspectorHexagonSizeMode != InspectorGridHexagonNodeSize.NodeSize)
                    GUILayout.Box("",
                        AstarPathEditor.astarSkin.FindStyle(
                            graph.inspectorHexagonSizeMode == InspectorGridHexagonNodeSize.Diameter
                                ? "HexagonDiameter"
                                : "HexagonWidth"));
            ...
            {
                newNodeSize = EditorGUILayout.FloatField(
                    new GUIContent("Node size",
                        "The size of a single node. The size is the side of the node square in world units"),
                    graph.nodeSize);
            }
            ...
            if (graph.inspectorGridMode == InspectorGridMode.IsometricGrid ||
                graph.inspectorGridMode == InspectorGridMode.Advanced)
            {
                graph.aspectRatio = EditorGUILayout.FloatField(
                    new GUIContent("Aspect Ratio", "Scaling of the nodes width/depth ratio. Good for isometric games"),
                    graph.aspectRatio);

                DrawIsometricField(graph);
            }

            if ((nodeSizeChanged && locked) || (newWidth != graph.width || newDepth != graph.depth) ||
                prevRatio != graph.aspectRatio)
            {
        ...
        void DrawWidthDepthFields(GridGraph graph, out int newWidth, out int newDepth)
        {
            ...
            newWidth = EditorGUILayout.IntField(new GUIContent("Width (nodes)", "Width of the graph in nodes"),
                graph.width);
            newDepth = EditorGUILayout.IntField(
                new GUIContent("Depth (nodes)", "Depth (or height you might also call it) of the graph in nodes"),
                graph.depth);
            ...
            locked = GUI.Toggle(lockRect, locked,
                new GUIContent("", "If the width and depth values are locked, " +
                                   ...
                                   "instead of keeping the size the same and changing the number of nodes in the graph"),
                lockStyle);
        }

        void DrawIsometricField(GridGraph graph)
        {
            var isometricGUIContent = new GUIContent("Isometric Angle",
                "For an isometric 2D game, you can use this parameter to scale the graph correctly.\nIt can also be used to create a hexagonal grid.\nYou may want to rotate the graph 45 degrees around the Y axis to make it line up better.");
            var isometricOptions = new[]
            {
                new GUIContent("None (0°)"), new GUIContent("Isometric (≈54.74°)"), new GUIContent("Dimetric (60°)"),
            ...
            isometricOption = EditorGUILayout.IntPopup(isometricGUIContent, isometricOption, isometricOptions,
                new[] { 0, 1, 2, 3 });
        ...
        void DrawPositionField(GridGraph graph)
        {
            ...
            var newWorldPoint =
                EditorGUILayout.Vector3Field(ObjectNames.NicifyVariableName(pivot.ToString()), worldPoint);
        ...
        protected virtual void DrawCutCorners(GridGraph graph)
        {
            ...
            graph.cutCorners =
                EditorGUILayout.Toggle(
                    new GUIContent("Cut Corners", "Enables or disables cutting corners. See docs for image example"),
                    graph.cutCorners);
        }
        ...
        protected virtual void DrawNeighbours(GridGraph graph)
        {
            ...
            var neighboursGUIContent = new GUIContent("Connections",
                "Sets how many connections a node should have to it's neighbour nodes.");
            ...
            graph.neighbours =
                (NumNeighbours)EditorGUILayout.Popup(neighboursGUIContent, (int)graph.neighbours, neighbourOptions);
            ...
            {
                graph.uniformEdgeCosts =
                    EditorGUILayout.Toggle(
                        new GUIContent("Hexagon connection costs",
                            "Tweak the edge costs in the graph to be more suitable for hexagon graphs"),
                        graph.uniformEdgeCosts);
                EditorGUILayout.HelpBox(
                    "You can set all settings to make this a hexagonal graph by changing the 'Mode' field above",
                    MessageType.None);
            }
            else
            {
        ...
        protected virtual void DrawMaxClimb(GridGraph graph)
        {
            if (!graph.collision.use2D)
            {
                graph.maxClimb = EditorGUILayout.FloatField(
                    new GUIContent("Max Climb",
                        "How high in world units, relative to the graph, should a climbable level be. A zero (0) indicates infinity"),
                    graph.maxClimb);
            ...
            if (!graph.collision.use2D)
            {
                graph.maxSlope =
                    EditorGUILayout.Slider(
                        new GUIContent("Max Slope",
                            "Sets the max slope in degrees for a point to be walkable. Only enabled if Height Testing is enabled."),
                        graph.maxSlope, 0, 90F);
            }
        }

        protected void DrawErosion(GridGraph graph)
        {
            graph.erodeIterations =
                EditorGUILayout.IntField(
                    new GUIContent("Erosion iterations",
                        "Sets how many times the graph should be eroded. This adds extra margin to objects."),
                    graph.erodeIterations);
            graph.erodeIterations =
                graph.erodeIterations < 0
                    ? 0
                    : (graph.erodeIterations > 16 ? 16 : graph.erodeIterations); //Clamp iterations to [0,16]

            if (graph.erodeIterations > 0)
            {
                EditorGUI.indentLevel++;
                graph.erosionUseTags = EditorGUILayout.Toggle(new GUIContent("Erosion Uses Tags",
                        "Instead of making nodes unwalkable, " +
        ...
        void DrawPenaltyModifications(GridGraph graph)
        {
            ...
            if (showExtra)
            {
                ...
                graph.penaltyAngle =
                    ToggleGroup(new GUIContent("Angle Penalty", "Adds a penalty based on the slope of the node"),
                        graph.penaltyAngle);
                if (graph.penaltyAngle)
                {
                    EditorGUI.indentLevel++;
                    graph.penaltyAngleFactor = EditorGUILayout.FloatField(
                        new GUIContent("Factor", "Scale of the penalty. A negative value should not be used"),
                        graph.penaltyAngleFactor);
                    graph.penaltyAnglePower = EditorGUILayout.Slider("Power", graph.penaltyAnglePower, 0.1f, 10f);
                    EditorGUILayout.HelpBox(
                        "Applies penalty to nodes based on the angle of the hit surface during the Height Testing\nPenalty applied is: P=(1-cos(angle)^power)*factor.",
                        MessageType.None);
                ...
                if (graph.penaltyPosition)
                {
                    ...
                    EditorGUILayout.HelpBox(
                        "Applies penalty to nodes based on their Y coordinate\nSampled in Int3 space, i.e it is multiplied with Int3.Precision first (" +
                        Int3.Precision + ")\n" +
        ...
        protected virtual void DrawCollisionEditor(GraphCollision collision)
        {
            ...
            {
                string[] colliderOptions =
                    collision.use2D ? new[] { "Circle", "Point" } : new[] { "Sphere", "Capsule", "Ray" };
        ...
        void DrawArc(Vector2 center, float radius, float startAngle, float endAngle)
        {
            ...
            for (int i = 0; i < arcBuffer.Length; i++)
            {
                ...
                arcBuffer[i] = new Vector3(center.x + radius * Mathf.Cos(angle), center.y + radius * Mathf.Sin(angle),
                    0);
            }
        ...
        void DrawCollisionPreview(GraphCollision collision)
        {
            ...
            int gridWidthInNodes = collision.type == ColliderType.Ray
                ? 3
                : Mathf.Max(3, RoundUpToNextOddNumber(collision.diameter + 0.5f));
            if (interpolatedGridWidthInNodes == -1) interpolatedGridWidthInNodes = gridWidthInNodes;
            if (Mathf.Abs(interpolatedGridWidthInNodes - gridWidthInNodes) < 0.01f)
                interpolatedGridWidthInNodes = gridWidthInNodes;
            else editor.Repaint();
            ...
            for (int i = 0; i <= gridWidthInNodes; i++)
            {
                ...
                DrawLine(gridCenter + new Vector2(c - 0.5f, -0.5f) * interpolatedGridScale,
                    gridCenter + new Vector2(c - 0.5f, 0.5f) * interpolatedGridScale);
                DrawLine(gridCenter + new Vector2(-0.5f, c - 0.5f) * interpolatedGridScale,
                    gridCenter + new Vector2(0.5f, c - 0.5f) * interpolatedGridScale);
            }
            ...
            {
                sideScale = Mathf.Max(scale * 0.5f,
                    Mathf.Min(scale, sideBase.y / (collision.height + collision.collisionOffset + diameter * 0.5f)));
                // A high collision offset should not cause it to break
                sideScale = Mathf.Min(sideScale, sideBase.y / (Mathf.Max(0, collision.collisionOffset) + diameter));
            }
            ...
            DrawLine(sideBase + new Vector2(-interpolatedGridSideScale * 0.5f, 0),
                sideBase + new Vector2(interpolatedGridSideScale * 0.5f, 0));
            for (int i = 0; i <= gridWidthInNodes; i++)
            {
                var c = i * 1.0f / gridWidthInNodes;
            ...
            if (collision.type == ColliderType.Ray)
            {
                ...
                DrawDashedLine(offset + new Vector2(0, -height) * sideScale,
                    offset + new Vector2(0, -height * 0.75f) * sideScale, dashLength);
            ...
            else
            {
                ...
                DrawArc(sideBase + new Vector2(0, -collision.collisionOffset * sideScale), diameter * 0.5f * sideScale,
                    0, Mathf.PI);
                DrawArc(sideBase + new Vector2(0, -(height + collision.collisionOffset) * sideScale),
                    diameter * 0.5f * sideScale, Mathf.PI, 2 * Mathf.PI);
                DrawDashedLine(sideBase + new Vector2(-diameter * 0.5f, -collision.collisionOffset) * sideScale,
        ...
        {
            collision.use2D =
                EditorGUILayout.Toggle(new GUIContent("Use 2D Physics", "Use the Physics2D API for collision checking"),
                    collision.use2D);
        }
        ...
        static readonly Vector3[] handlePoints = new[]
        {
            new Vector3(0.0f, 0, 0.5f), new Vector3(1.0f, 0, 0.5f), new Vector3(0.5f, 0, 0.0f),
        ...
        public override void OnSceneGUI(NavGraph target)
        {
            ...
            if (Tools.current == Tool.Scale)
            {
                ...
                for (int i = 0; i < handlePoints.Length; i++)
                {
                    ...
                    Vector3 p = savedTransform.InverseTransform(Handles.Slider(ps, ps - center,
                        HandleScale * HandleUtility.GetHandleSize(ps), cap, 0));
                ...
                if (EditorGUI.EndChangeCheck())
                {
                    ...
                    graph.unclampedSize = Vector2.Scale(new Vector2(mx.x - mn.x, mx.z - mn.z), savedDimensions) *
                                          savedNodeSize;
                }
            }
            ...
            {
                EditorGUI.BeginChangeCheck();
                center = Handles.PositionHandle(graph.center,
                    Tools.pivotRotation == PivotRotation.Global
                        ? Quaternion.identity
                        : Quaternion.Euler(graph.rotation));
