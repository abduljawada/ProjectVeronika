...
namespace Pathfinding
{
    ...
    public class GridGraph : NavGraph, IUpdatableGraph, ITransformedGraph
    {
        ...
        {
            get { return Math.Abs(90 - maxSlope) > float.Epsilon; }
        }
        ...
        /// See: erosionUseTags
        /// </summary>
        [JsonMember] public int erodeIterations;
        ...
        /// See: erosionFirstTag
        /// </summary>
        [JsonMember] public bool erosionUseTags;
        ...
        /// </summary>
        [JsonMember] public int erosionFirstTag = 1;
        ...
        /// </summary>
        [JsonMember] public NumNeighbours neighbours = NumNeighbours.Eight;
        ...
        /// </summary>
        [JsonMember] public bool cutCorners = true;

        /// <summary>
        /// Offset for the position when calculating penalty.
        /// See: penaltyPosition
        /// </summary>
        [JsonMember] public float penaltyPositionOffset;

        /// <summary>Use position (y-coordinate) to calculate penalty</summary>
        [JsonMember] public bool penaltyPosition;

        /// <summary>
        /// Scale factor for penalty when calculating from position.
        /// See: penaltyPosition
        /// </summary>
        [JsonMember] public float penaltyPositionFactor = 1F;

        [JsonMember] public bool penaltyAngle;
        ...
        [JsonMember] public bool showMeshSurface = true;
        ...
        /// </code>
        /// </summary>
        [System.NonSerialized] public readonly int[] neighbourOffsets = new int[8];

        /// <summary>Costs to neighbour nodes</summary>
        [System.NonSerialized] public readonly uint[] neighbourCosts = new uint[8];

        /// <summary>Offsets in the X direction for neighbour nodes. Only 1, 0 or -1</summary>
        [System.NonSerialized] public readonly int[] neighbourXOffsets = new int[8];

        /// <summary>Offsets in the Z direction for neighbour nodes. Only 1, 0 or -1</summary>
        [System.NonSerialized] public readonly int[] neighbourZOffsets = new int[8];
        ...
        public bool is2D
        {
            get { return Quaternion.Euler(this.rotation) * Vector3.up == -Vector3.forward; }
            set
            {
                if (value != is2D)
                {
                    this.rotation = value
                        ? new Vector3(this.rotation.y - 90, 270, 90)
                        : new Vector3(0, this.rotation.x + 90, 0);
                }
            }
        }
        ...
        public override void RelocateNodes(Matrix4x4 deltaMatrix)
        {
            ...
            throw new System.Exception(
                "This method cannot be used for Grid Graphs. Please use the other overload of RelocateNodes instead");
        }
        ...
        /// </summary>
        public void RelocateNodes(Vector3 center, Quaternion rotation, float nodeSize, float aspectRatio = 1,
        ...
        public GraphTransform CalculateTransform()
        {
            ...
            isometricMatrix = Matrix4x4.Scale(new Vector3(Mathf.Cos(Mathf.Deg2Rad * isometricAngle), 1, 1)) *
                              isometricMatrix;
            ...
            var boundsMatrix = Matrix4x4.TRS(center, Quaternion.Euler(rotation), new Vector3(aspectRatio, 1, 1)) *
                               isometricMatrix;
            ...
            var m = Matrix4x4.TRS(
                boundsMatrix.MultiplyPoint3x4(-new Vector3(newWidth * newNodeSize, 0, newDepth * newNodeSize) * 0.5F),
                Quaternion.Euler(rotation), new Vector3(newNodeSize * aspectRatio, 1, newNodeSize)) * isometricMatrix;
        ...
        public override NNInfoInternal GetNearest(Vector3 position, NNConstraint constraint, GraphNode hint)
        {
            ...
            nn.clampedPosition =
                transform.Transform(new Vector3(Mathf.Clamp(xf, x, x + 1f), y, Mathf.Clamp(zf, z, z + 1f)));
        ...
        public override NNInfoInternal GetNearestForce(Vector3 position, NNConstraint constraint)
        {
            ...
            float maxDist = constraint == null || constraint.constrainDistance
                ? AstarPath.active.maxNearestNodeDistance
                ...
                for (nx = x - w; nx <= x + w; nx++)
                {
                    ...
                    if (constraint == null || constraint.Suitable(nodes[nx + nz2]))
                    {
                        ...
                        if (dist < minDist && dist < maxDistSqr)
                        {
                            ...
                            clampedPosition = transform.Transform(new Vector3(Mathf.Clamp(xf, nx, nx + 1f),
                                transform.InverseTransform((Vector3)minNode.position).y, Mathf.Clamp(zf, nz, nz + 1f)));
                        }
                    }
                }
        ...
        public virtual void SetUpOffsetsAndCosts()
        {
            ...
            uint diagonalCost = uniformEdgeCosts
                ? straightCost
                : (uint)Mathf.RoundToInt(nodeSize * Mathf.Sqrt(2F) * Int3.Precision);
        ...
        public virtual void RecalculateCell(int x, int z, bool resetPenalties = true, bool resetTags = true)
        {
            ...
            if (resetPenalties)
            {
                ...
                {
                    node.Penalty +=
                        (uint)Mathf.RoundToInt((node.position.y - penaltyPositionOffset) * penaltyPositionFactor);
                }
            }
        ...
        public void ErodeWalkableArea(int xmin, int zmin, int xmax, int zmax)
        {
            if (erosionUseTags)
            {
                ...
                {
                    Debug.LogError(
                        "Too few tags available for " + erodeIterations + " erode iterations and starting with tag " +
                        erosionFirstTag + " (erodeIterations+erosionFirstTag > 31)", active);
                    return;
                }
        ...
        public override void OnDrawGizmos(RetainedGizmos gizmos, bool drawNodes)
        {
            using (var helper = gizmos.GetSingleFrameGizmoHelper(active))
            {
                ...
                if (drawNodes && width * depth * LayerCount != nodeCount)
                {
                    ...
                    for (int z = 0; z < d; z++)
                    {
                        helper.builder.DrawLine(trans.Transform(new Vector3(0, 0, z)),
                            trans.Transform(new Vector3(w, 0, z)), color);
                    }

                    for (int x = 0; x < w; x++)
                    {
                        helper.builder.DrawLine(trans.Transform(new Vector3(x, 0, 0)),
                            trans.Transform(new Vector3(x, 0, d)), color);
                    }
                }
            }
            ...
            for (int cx = width / chunkWidth; cx >= 0; cx--)
            {
                for (int cz = depth / chunkWidth; cz >= 0; cz--)
                {
                    ...
                    var allNodesCount =
                        GetNodesInRegion(
                            new IntRect(cx * chunkWidth, cz * chunkWidth, (cx + 1) * chunkWidth - 1,
                                (cz + 1) * chunkWidth - 1), allNodes);
                    ...
                    if (!gizmos.Draw(hasher))
                    {
                        ...
                        using (var helper = gizmos.GetGizmoHelper(active, hasher))
                        {
                            ...
                            if (showMeshSurface || showMeshOutline)
                                CreateNavmeshSurfaceVisualization(allNodes, allNodesCount, helper);
                        }
        ...
        void CreateNavmeshSurfaceVisualization(GridNodeBase[] nodes, int nodeCount, GraphGizmoHelper helper)
        {
            ...
            for (int i = 0; i < nodeCount; i++)
            {
                ...
                for (int j = 0; j < neighbourIndices.Length; j++)
                {
                    ...
                    {
                        helper.builder.DrawLine(vertices[baseIndex + j],
                            vertices[baseIndex + (j + 1) % neighbourIndices.Length],
                            other == null ? Color.black : nodeColor);
                    }
                }
            ...
            for (int x = rect.xmin; x <= rect.xmax; x++)
            {
                for (int z = rect.ymin; z <= rect.ymax; z++)
                {
                    ...
                    if (bounds.Contains((Vector3)node.position) &&
                        (shape == null || shape.Contains((Vector3)node.position)))
                    {
                        inArea.Add(node);
                    }
                }
        ...
        protected void CalculateAffectedRegions(GraphUpdateObject o, out IntRect originalRect, out IntRect affectRect,
            out IntRect physicsRect, out bool willChangeWalkability, out int erosion)
        {
            // Take the bounds and transform it using the matrix
            // Then convert that to a rectangle which contains
            // all nodes that might be inside the bounds
        ...
        void IUpdatableGraph.UpdateArea(GraphUpdateObject o)
        {
            ...
            int erosion;
            CalculateAffectedRegions(o, out originalRect, out affectRect, out physicsRect, out willChangeWalkability,
                out erosion);
        ...
        protected override void PostDeserialization(GraphSerializationContext ctx)
        {
            ...
            for (int z = 0; z < depth; z++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    {
                        Debug.LogError(
                            "Deserialization Error : Couldn't cast the node to the appropriate type - GridGenerator");
                        return;
                    }
