...
namespace Pathfinding
{
    ...
    public class BBTree : IAstarPooledObject
    {
        ...
        public Rect Size
        {
            get
            {
                ...
                else
                {
                    ...
                    return Rect.MinMaxRect(rect.xmin * Int3.PrecisionFactor, rect.ymin * Int3.PrecisionFactor,
                        rect.xmax * Int3.PrecisionFactor, rect.ymax * Int3.PrecisionFactor);
                }
            }
        }
        ...
        int RebuildFromInternal(TriangleMeshNode[] nodes, int[] permutation, IntRect[] nodeBounds, int from, int to,
            bool odd)
        {
        ...
        public NNInfoInternal QueryClosestXZ(Vector3 p, NNConstraint constraint, ref float distance,
        ...
        public NNInfoInternal QueryClosest(Vector3 p, NNConstraint constraint, ref float distance,
            NNInfoInternal previous)
        {
            var sqrDistance = distance * distance;
            var origSqrDistance = sqrDistance;
        ...
        void SearchBoxClosest(int boxi, Vector3 p, ref float closestSqrDist, NNConstraint constraint,
            ref NNInfoInternal nnInfo)
        {
            BBTreeBox box = tree[boxi];
