...
namespace Pathfinding
{
    ...
    public abstract class NavmeshBase : NavGraph, INavmesh, INavmeshHolder, ITransformedGraph
    {
        ...
        [JsonMember] public Vector3 forcedBoundsSize = new Vector3(100, 40, 100);
        ...
        /// </summary>
        [JsonMember] public bool nearestSearchOnlyXZ;

        /// <summary>
        /// Should navmesh cuts affect this graph.
        /// See: <see cref="navmeshUpdateData"/>
        /// </summary>
        [JsonMember] public bool enableNavmeshCutting = true;
        ...
        public IntRect GetTouchingTiles(Bounds bounds, float margin = 0)
        {
            ...
            var r = new IntRect(Mathf.FloorToInt((bounds.min.x - margin) / TileWorldSizeX),
                Mathf.FloorToInt((bounds.min.z - margin) / TileWorldSizeZ),
                Mathf.FloorToInt((bounds.max.x + margin) / TileWorldSizeX),
                Mathf.FloorToInt((bounds.max.z + margin) / TileWorldSizeZ));
            // Clamp to bounds
        ...
        {
            // Calculate world bounds of all affected tiles
            var r = new IntRect(Mathf.FloorToInt(rect.xMin / TileWorldSizeX),
                Mathf.FloorToInt(rect.yMin / TileWorldSizeZ), Mathf.FloorToInt(rect.xMax / TileWorldSizeX),
                Mathf.FloorToInt(rect.yMax / TileWorldSizeZ));
        ...
        public IntRect GetTouchingTilesRound(Bounds bounds)
        {
            ...
            //Calculate world bounds of all affected tiles
            var r = new IntRect(Mathf.RoundToInt(bounds.min.x / TileWorldSizeX),
                Mathf.RoundToInt(bounds.min.z / TileWorldSizeZ), Mathf.RoundToInt(bounds.max.x / TileWorldSizeX) - 1,
                Mathf.RoundToInt(bounds.max.z / TileWorldSizeZ) - 1);
            //Clamp to bounds
            r = IntRect.Intersection(r, new IntRect(0, 0, tileXCount - 1, tileZCount - 1));
            return r;
        }
        ...
        protected void ConnectTileWithNeighbours(NavmeshTile tile, bool onlyUnflagged = false)
        {
            if (tile.w != 1 || tile.d != 1)
            {
                throw new System.ArgumentException(
                    "Tile widths or depths other than 1 are not supported. The fields exist mainly for possible future expansions.");
            }
        ...
        {
            return GetNearestForce(position,
                constraint != null && constraint.distanceXZ ? NNConstraintDistanceXZ : null);
        }
        ...
        public GraphNode PointOnNavmesh(Vector3 position, NNConstraint constraint)
        {
            ...
            // Graph borders
            if (tileCoords.x < 0 || tileCoords.y < 0 || tileCoords.x >= tileXCount ||
                tileCoords.y >= tileZCount) return null;
        ...
        protected void ConnectTiles(NavmeshTile tile1, NavmeshTile tile2)
        {
            ...
            {
                throw new System.ArgumentException(
                    "Tiles are not adjacent (tile coordinates must differ by exactly 1. Got '" + t1coord + "' and '" +
                    t2coord + "')");
            }
                    ...
                    if (Math.Abs(aVertex1[coord] - midpoint) < 2 && Math.Abs(aVertex2[coord] - midpoint) < 2)
                    {
                        ...
                        for (int j = 0; j < numCloseToEdge; j++)
                        {
                            ...
                            for (int b = 0; b < bVertexCount; b++)
                            {
                                Int3 bVertex1 = nodeB.GetVertexInGraphSpace(b);
                                Int3 bVertex2 = nodeB.GetVertexInGraphSpace((b + 1) % bVertexCount);
                                if (Math.Abs(bVertex1[coord] - midpoint) < 2 &&
                                    Math.Abs(bVertex2[coord] - midpoint) < 2)
                                {
                                    ...
                                    if (maxalt > minalt2 && minalt < maxalt2)
                                    {
                                        ...
                                        if ((aVertex1 == bVertex1 && aVertex2 == bVertex2) ||
                                            (aVertex1 == bVertex2 && aVertex2 == bVertex1) ||
                                            VectorMath.SqrDistanceSegmentSegment((Vector3)aVertex1, (Vector3)aVertex2,
        ...
        {
            if (batchTileUpdate)
                throw new System.InvalidOperationException("Calling StartBatchLoad when batching is already enabled");
        ...
        public void EndBatchTileUpdate()
        {
            if (!batchTileUpdate)
                throw new System.InvalidOperationException(
                    "Calling EndBatchTileUpdate when batching had not yet been started");
        ...
        {
            if (!batchTileUpdate)
                throw new System.Exception("Must be called during a batch update. See StartBatchTileUpdate");
        ...
        void PrepareNodeRecycling(int x, int z, Int3[] verts, int[] tris, TriangleMeshNode[] recycledNodeBuffer)
        {
            ...
            {
                recycling[
                    verts[tris[i + 0]].GetHashCode() + verts[tris[i + 1]].GetHashCode() +
                    verts[tris[i + 2]].GetHashCode()] = j;
            }
            ...
            for (int i = 0; i < nodes.Length; i++)
            {
                ...
                if (recycling.TryGetValue(hash, out newNodeIndex))
                {
                    ...
                    if (verts[tris[3 * newNodeIndex + 0]] == v0 && verts[tris[3 * newNodeIndex + 1]] == v1 &&
                        verts[tris[3 * newNodeIndex + 2]] == v2)
                    {
        ...
        public void ReplaceTile(int x, int z, Int3[] verts, int[] tris)
        {
            ...
            {
                throw new System.ArgumentException(
                    "Tile is placed at an out of bounds position or extends out of the graph bounds (" + x + ", " + z +
                    " [" + w + ", " + d + "] " + tileXCount + " " + tileZCount + ")");
            }
            ...
            if (!Mathf.Approximately(x * TileWorldSizeX * Int3.FloatPrecision,
                    (float)Math.Round(x * TileWorldSizeX * Int3.FloatPrecision)))
                Debug.LogWarning("Possible numerical imprecision. Consider adjusting tileSize and/or cellSize");
            if (!Mathf.Approximately(z * TileWorldSizeZ * Int3.FloatPrecision,
                    (float)Math.Round(z * TileWorldSizeZ * Int3.FloatPrecision)))
                Debug.LogWarning("Possible numerical imprecision. Consider adjusting tileSize and/or cellSize");
        ...
        protected void CreateNodes(TriangleMeshNode[] buffer, int[] tris, int tileIndex, uint graphIndex)
        {
            if (buffer == null || buffer.Length < tris.Length / 3)
                throw new System.ArgumentException("buffer must be non null and at least as large as tris.Length/3");
            ...
            for (int i = 0; i < buffer.Length; i++)
            {
                ...
                if (RecalculateNormals && !VectorMath.IsClockwiseXZ(node.GetVertexInGraphSpace(0),
                        node.GetVertexInGraphSpace(1), node.GetVertexInGraphSpace(2)))
                {
        ...
        static void CreateNavmeshOutlineVisualization(NavmeshTile[] tiles, int startTile, int endTile,
            ...
            for (int i = startTile; i < endTile; i++)
            {
                ...
                for (int j = 0; j < tile.nodes.Length; j++)
                {
                    ...
                    for (int c = 0; c < node.connections.Length; c++)
                    {
                        ...
                        if (other != null && other.GraphIndex == node.GraphIndex)
                        {
                            for (int v = 0; v < 3; v++)
                            {
                                ...
                                {
                                    if (node.GetVertexIndex(v) == other.GetVertexIndex((v2 + 1) % 3) &&
                                        node.GetVertexIndex((v + 1) % 3) == other.GetVertexIndex(v2))
                                    {
                    ...
                    for (int v = 0; v < 3; v++)
                    {
                        if (!sharedEdges[v])
                        {
                            helper.builder.DrawLine((Vector3)node.GetVertex(v), (Vector3)node.GetVertex((v + 1) % 3),
                                color);
                        }
                    }
                }
                ...
                for (int x = 0; x < tileXCount; x++)
                {
                    ...
                    if (trisCount % 3 != 0)
                        throw new System.Exception(
                            "Corrupt data. Triangle indices count must be divisable by 3. Read " + trisCount);
                    ...
                    if (ctx.meta.version.Major >= 4)
                    {
                        ...
                        if (tile.vertsInGraphSpace.Length != tile.verts.Length)
                            throw new System.Exception("Corrupt data. Array lengths did not match");
                        for (int i = 0; i < tile.verts.Length; i++)
                        {
        ...
        protected override void PostDeserialization(GraphSerializationContext ctx)
        {
            ...
            {
                Dictionary<TriangleMeshNode, Connection[]> conns = tiles.SelectMany(s => s.nodes)
                    .ToDictionary(n => n, n => n.connections ?? new Connection[0]);
