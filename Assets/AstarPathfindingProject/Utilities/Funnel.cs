...
namespace Pathfinding
{
    ...
    public class Funnel
    {
        ...
        public static List<PathPart> SplitIntoParts(Path path)
        {
            ...
            for (int i = 0; i < nodes.Count; i++)
            {
                ...
                else if (NodeLink2.GetNodeLink(nodes[i]) != null)
                {
                    ...
                    {
                        throw new System.Exception("NodeLink2 link length greater than two (2) nodes. " +
                                                   (i - part.startIndex + 1));
                    }
        ...
        static bool UnwrapHelper(Vector3 portalStart, Vector3 portalEnd, Vector3 prevPoint, Vector3 nextPoint,
            ref Quaternion mRot, ref Vector3 mOffset)
        {
        ...
        public static void Unwrap(FunnelPortals funnel, Vector2[] left, Vector2[] right)
        {
            ...
            {
                startingIndex++;
                normal = Vector3.Cross(funnel.right[startingIndex] - funnel.left[0],
                    funnel.left[startingIndex] - funnel.left[0]);
            }
        ...
        static int FixFunnel(Vector2[] left, Vector2[] right, int numPortals)
        {
            if (numPortals > left.Length || numPortals > right.Length)
                throw new System.ArgumentException("Arrays do not have as many elements as specified");
        ...
        public static List<Vector3> Calculate(FunnelPortals funnel, bool unwrap, bool splitAtEveryPortal)
        {
            ...
            {
                bool lastCorner;
                Calculate(leftArr, rightArr, funnel.left.Count, startIndex, intermediateResult, int.MaxValue,
                    out lastCorner);
            }
            ...
            for (int i = 0; i < intermediateResult.Count; i++)
            {
                ...
                if (splitAtEveryPortal)
                {
                    ...
                    {
                        var factor = VectorMath.LineIntersectionFactorXZ(FromXZ(leftArr[j]), FromXZ(rightArr[j]),
                            FromXZ(prev2D), FromXZ(next2D));
        ...
        /// See: http://digestingduck.blogspot.se/2010/03/simple-stupid-funnel-algorithm.html
        /// </summary>
        static void Calculate(Vector2[] left, Vector2[] right, int numPortals, int startIndex, List<int> funnelPath,
            int maxCorners, out bool lastCorner)
        {
