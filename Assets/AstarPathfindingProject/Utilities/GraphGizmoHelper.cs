...
namespace Pathfinding.Util
{
    public class GraphGizmoHelper : IAstarPooledObject, System.IDisposable
    {
        ...
        public void DrawConnections(GraphNode node)
        {
            if (showSearchTree)
            {
                if (InSearchTree(node, debugData, debugPathID))
                {
                    ...
                    {
                        builder.DrawLine((Vector3)node.position,
                            (Vector3)debugData.GetPathNode(node).parent.node.position, NodeColor(node));
                    }
                }
            }
            else
        ...
        {
            builder.DrawLine(drawConnectionStart, Vector3.Lerp((Vector3)other.position, drawConnectionStart, 0.5f),
                drawConnectionColor);
        }
