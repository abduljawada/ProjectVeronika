    ...
    public class RetainedGizmos
    {
        ...
        public struct Hasher
        {
            ...
            public Hasher(AstarPath active)
            {
                ...
                includePathSearchInfo = debugData != null && (active.debugMode == GraphDebugMode.F ||
                                                              active.debugMode == GraphDebugMode.G ||
                                                              active.debugMode == GraphDebugMode.H ||
        ...
        public void FinalizeDraw()
        {
            ...
            if (surfaceMaterial == null)
                surfaceMaterial =
                    UnityEditor.AssetDatabase.LoadAssetAtPath(
                        EditorResourceHelper.editorAssets + "/Materials/Navmesh.mat", typeof(Material)) as Material;
            if (lineMaterial == null)
                lineMaterial =
                    UnityEditor.AssetDatabase.LoadAssetAtPath(
                        EditorResourceHelper.editorAssets + "/Materials/NavmeshOutline.mat",
            ...
            for (int matIndex = 0; matIndex <= 1; matIndex++)
            {
                ...
                for (int pass = 0; pass < mat.passCount; pass++)
                {
                    ...
                    for (int i = 0; i < meshes.Count; i++)
                    {
                        if (meshes[i].lines == (mat == lineMaterial) &&
                            GeometryUtility.TestPlanesAABB(planes, meshes[i].mesh.bounds))
                        {
