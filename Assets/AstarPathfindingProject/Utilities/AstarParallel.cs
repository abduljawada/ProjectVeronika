...
namespace Pathfinding.Util
{
    ...
    public class ParallelWorkQueue<T>
    {
        ...
        public ParallelWorkQueue(Queue<T> queue)
        {
			...
			threadCount = 1;
#else
            threadCount = System.Math.Min(initialCount,
                System.Math.Max(1, AstarPath.CalculateThreadCount(ThreadCount.AutomaticHighLoad)));
#endif
        }
        ...
        public IEnumerable<int> Run(int progressTimeoutMillis)
        {
            if (initialCount != queue.Count)
                throw new System.InvalidOperationException("Queue has been modified since the constructor");
