...
namespace Pathfinding
{
    ...
    public class PathProcessor
    {
        ...
        public struct GraphUpdateLock
        {
            ...
            public bool Held
            {
                get { return pathProcessor != null && pathProcessor.locks.Contains(id); }
            }
        ...
        public void JoinThreads()
        {
            if (threads != null)
            {
                for (int i = 0; i < threads.Length; i++)
                {
                    ...
                    {
                        Debug.LogError(
                            "Could not terminate pathfinding thread[" + i + "] in 200ms, trying Thread.Abort");
                        threads[i].Abort();
                    }
                }
            }
        ...
        public void InitializeNode(GraphNode node)
        {
            if (!queue.AllReceiversBlocked)
            {
                throw new System.Exception(
                    "Trying to initialize a node when it is not safe to initialize any nodes. Must be done during a graph update. See http://arongranberg.com/astar/docs/graph-updates.php#direct");
            }
        ...
        {
#if UNITY_2017_3_OR_NEWER
            UnityEngine.Profiling.Profiler.BeginThreadProfiling("Pathfinding",
                "Pathfinding thread #" + (pathHandler.threadID + 1));
#endif
