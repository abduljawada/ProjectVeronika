...
namespace Pathfinding
{
    ...
    public static class GraphUtilities
    {
        ...
        public static void GetContours(GridGraph grid, System.Action<Vector3[]> callback, float yMergeThreshold,
            GridNodeBase[] nodes = null)
        {
            ...
            var neighbourIndices = grid.neighbours == NumNeighbours.Six
                ? GridGraph.hexagonNeighbourIndices
                : new[] { 0, 1, 2, 3 };
            ...
            if (nodes != null)
            {
                ...
                for (int i = 0; i < nodes.Length; i++)
                {
                    ...
                    if (startNode != null && startNode.Walkable &&
                        (!startNode.HasConnectionsToAllEightNeighbours || nodeSet != null))
                    {
                        for (int startDir = 0; startDir < neighbourIndices.Length; startDir++)
                        {
                            ...
                            if ((startNeighbour == null || (nodeSet != null && !nodeSet.Contains(startNeighbour))) &&
                                !seenStates.Contains(startState))
                            {
                                ...
                                while (true)
                                {
                                    ...
                                    if (neighbour == null || (nodeSet != null && !nodeSet.Contains(neighbour)))
                                    {
                                        ...
                                        Vector3 graphSpacePos = new Vector3(node.XCoordinateInGrid + 0.5f, 0,
                                            node.ZCoordinateInGrid + 0.5f);
                                        // Offset along diagonal to get the correct XZ coordinates
                                        graphSpacePos.x += (neighbourXOffsets[d0] + neighbourXOffsets[d1]) *
                                                           offsetMultiplier;
                                        graphSpacePos.z += (neighbourZOffsets[d0] + neighbourZOffsets[d1]) *
                                                           offsetMultiplier;
                                        ...
                                        if (trace.Count >= 2)
                                        {
                                            ...
                                            if (((Mathf.Abs(v1d.x) > 0.01f || Mathf.Abs(v2d.x) > 0.01f) &&
                                                 (Mathf.Abs(v1d.z) > 0.01f || Mathf.Abs(v2d.z) > 0.01f)) ||
                                                (Mathf.Abs(v1d.y) > yMergeThreshold ||
                                                 Mathf.Abs(v2d.y) > yMergeThreshold))
                                            {
                                ...
                                if (trace.Count >= 3)
                                {
                                    ...
                                    if (!(((Mathf.Abs(v1d.x) > 0.01f || Mathf.Abs(v2d.x) > 0.01f) &&
                                           (Mathf.Abs(v1d.z) > 0.01f || Mathf.Abs(v2d.z) > 0.01f)) ||
                                          (Mathf.Abs(v1d.y) > yMergeThreshold || Mathf.Abs(v2d.y) > yMergeThreshold)))
                                    {
