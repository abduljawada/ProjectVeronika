    ...
    public class HierarchicalGraph
    {
        ...
        public HierarchicalGraph()
        {
            ...
            connectionCallback = (GraphNode neighbour) =>
            {
                ...
                if (hIndex == 0)
                {
                    if (currentChildren.Count < MaxChildrenPerNode &&
                        neighbour
                            .Walkable /* && (((GridNode)currentChildren[0]).XCoordinateInGrid/Tiling == ((GridNode)neighbour).XCoordinateInGrid/Tiling) && (((GridNode)currentChildren[0]).ZCoordinateInGrid/Tiling == ((GridNode)neighbour).ZCoordinateInGrid/Tiling)*/
                       )
                    {
        ...
        internal void AddDirtyNode(GraphNode node)
        {
            if (!node.IsHierarchicalNodeDirty)
            {
                ...
                else
                {
                    ...
                    if (numDirtyNodes >= dirtyNodes.Length)
                        throw new System.Exception("Failed to compactify dirty nodes array. This should not happen. " +
                                                   maxIndex + " " + numDirtyNodes + " " + dirtyNodes.Length);
                    AddDirtyNode(node);
                }
            }
        }
