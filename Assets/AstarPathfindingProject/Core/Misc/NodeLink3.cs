...
namespace Pathfinding
{
    ...
    public class NodeLink3Node : PointNode
    {
        ...
        public override bool GetPortal(GraphNode other, List<Vector3> left, List<Vector3> right, bool backwards)
        {
            ...
            if (this.connections.Length != 2)
                throw new System.Exception("Invalid NodeLink3Node. Expected 2 connections, found " +
        ...
        public GraphNode GetOther(GraphNode a)
        {
            ...
            return a == connections[0].node
                ? (connections[1].node as NodeLink3Node).GetOtherInternal(this)
                : (connections[0].node as NodeLink3Node).GetOtherInternal(this);
        }
    ...
    public class NodeLink3 : GraphModifier
    {
        ...
        public void InternalOnPostScan()
        {
            ...
            startNode = AstarPath.active.data.pointGraph.AddNode(new NodeLink3Node(AstarPath.active),
                (Int3)StartTransform.position);
            startNode.link = this;
            endNode = AstarPath.active.data.pointGraph.AddNode(new NodeLink3Node(AstarPath.active),
                (Int3)EndTransform.position);
            endNode.link = this;
#else
...
#if !ASTAR_NO_POINT_GRAPH
            if (Application.isPlaying && AstarPath.active != null && AstarPath.active.data != null &&
        ...
        public void Apply(bool forceNewCheck)
        {
            ...
            uint cost = (uint)Mathf.RoundToInt(((Int3)(StartTransform.position - EndTransform.position)).costMagnitude *
                                               costFactor);
        ...
        private readonly static Color GizmosColorSelected =
            new Color(235.0f / 255.0f, 123.0f / 255.0f, 32.0f / 255.0f, 1.0f);
        ...
        public void OnDrawGizmos(bool selected)
        {
            ...
            if (StartTransform != null && EndTransform != null)
            {
                ...
                if (selected)
                {
                    Vector3 cross = Vector3.Cross(Vector3.up, (EndTransform.position - StartTransform.position))
                        .normalized;
                    Draw.Gizmos.Bezier(StartTransform.position + cross * 0.1f, EndTransform.position + cross * 0.1f,
                        col);
                    Draw.Gizmos.Bezier(StartTransform.position - cross * 0.1f, EndTransform.position - cross * 0.1f,
                        col);
                }
