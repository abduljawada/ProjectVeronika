...
namespace Pathfinding.Serialization
{
    ...
    public class GraphSerializationContext
    {
        ...
        public UnityEngine.Object DeserializeUnityObject()
        {
            ...
            {
                UnityReferenceHelper[] helpers =
                    UnityEngine.Object.FindObjectsOfType(typeof(UnityReferenceHelper)) as UnityReferenceHelper[];
    ...
    public class AstarSerializer
    {
        ...
        public AstarSerializer(AstarData data, GameObject contextRoot) : this(data, SerializeSettings.Settings,
            contextRoot)
        {
        }
        ...
        static byte[] SerializeNodeIndices(NavGraph[] graphs)
        {
            ...
            if (maxNodeIndex2 != maxNodeIndex)
                throw new Exception(
                    "Some graphs are not consistent in their GetNodes calls, sequential calls give different results.");
        ...
        public void SerializeExtraInfo()
        {
            ...
            if (graphs == null)
                throw new InvalidOperationException(
                    "Cannot serialize extra info with no serialized graphs (call SerializeGraphs first)");
        ...
        GraphNode[] DeserializeNodeReferenceMap()
        {
            ...
            if (entry == null)
                throw new Exception(
                    "Node references not found in the data. Was this loaded from an older version of the A* Pathfinding Project?");
            ...
            {
                throw new Exception(
                    "Some graph(s) has thrown an exception during GetNodes, or some graph(s) have deserialized more or fewer nodes than were serialized",
                    e);
            }
            ...
            {
                throw new Exception((reader.BaseStream.Length / 4) + " nodes were serialized, but only data for " +
                                    (reader.BaseStream.Position / 4) + " nodes was found. The data looks corrupt.");
            }
#endif
        ...
        void DeserializeNodeReferences(NavGraph graph, GraphNode[] int2Node)
        {
            ...
            if (entry == null)
                throw new Exception("Node references for graph " + zipIndex +
                                    " not found in the data. Was this loaded from an older version of the A* Pathfinding Project?");
