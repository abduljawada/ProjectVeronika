...
namespace Pathfinding
{
    ...
    public abstract class Path : IPathInternals
    {
        ...
        public bool error
        {
            get { return CompleteState == PathCompleteState.Error; }
        }
        ...
        internal uint CalculateHScore(GraphNode node)
        {
            ...
            switch (heuristic)
            {
                ...
                case Heuristic.Manhattan:
                    ...
                    h = (uint)((System.Math.Abs(hTarget.x - p2.x) + System.Math.Abs(hTarget.y - p2.y) +
                                System.Math.Abs(hTarget.z - p2.z)) * heuristicScale);
                    return h;
        ...
        private void ErrorCheck()
        {
            if (!hasBeenReset)
                FailWithError(
                    "Please use the static Construct function for creating paths, do not use the normal constructors.");
            ...
            if (PipelineState > PathState.Processing)
                FailWithError(
                    "This path has already been processed. Do not request a path with the same path object twice.");
        }
        ...
        public void Claim(System.Object o)
        {
            ...
            for (int i = 0; i < claimed.Count; i++)
            {
                ...
                if (System.Object.ReferenceEquals(claimed[i], o))
                    throw new System.ArgumentException("You have already claimed the path with that object (" + o +
                                                       "). Are you claiming the path with the same object twice?");
            }
        ...
        public void Release(System.Object o, bool silent = false)
        {
            ...
            if (claimed.Count == 0)
            {
                throw new System.ArgumentException(
                    "You are releasing a path which is not claimed at all (most likely it has been pooled already). " +
                    "Are you releasing the path with the same object (" + o + ") twice?" +
                    "\nCheck out the documentation on path pooling for help.");
            }

            throw new System.ArgumentException(
                "You are releasing a path which has not been claimed with this object (" + o + "). " +
