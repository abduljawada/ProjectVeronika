...
namespace Pathfinding
{
    ...
    public class AstarData
    {
        ...
        /// </summary>
        [SerializeField] public bool cacheStartup;
            ...
            if (graphStructureLocked.Count > 0)
            {
                ...
                if (!(onlyAddingGraph && allowAdding))
                    throw new System.InvalidOperationException(
                        "Graphs cannot be added, removed or serialized while the graph structure is locked. This is the case when a graph is currently being scanned and when executing graph updates and work items.\nHowever as a special case, graphs can be added inside work items.");
            }
        ...
        public void DeserializeGraphsAdditive(byte[] bytes)
        {
            ...
            try
            {
                if (bytes != null)
                {
                    ...
                    {
                        Debug.Log(
                            "Invalid data file (cannot read zip).\nThe data is either corrupt or it was saved using a 3.0.x or earlier version of the system");
                    }
                }
                else
                {
        ...
        public NavGraph AddGraph(System.Type type)
        {
            ...
            if (graph == null)
            {
                Debug.LogError("No NavGraph of type '" + type + "' could be found, " + graphTypes.Length +
        ...
        {
            return FindGraph(graph =>
                WindowsStoreCompatibility.GetTypeInfo(type)
                    .IsAssignableFrom(WindowsStoreCompatibility.GetTypeInfo(graph.GetType())));
        }
        ...
        /// </summary>
        [System.Obsolete(
            "Obsolete because it is not used by the package internally and the use cases are few. Iterate through the graphs array instead.")]
