...
namespace Pathfinding
{
    ...
    public class AILerp : VersionedMonoBehaviour, IAstarAI
    {
        ...
        public bool canSearch
        {
            ...
            set { this.autoRepath.mode = value ? AutoRepathPolicy.Mode.EveryNSeconds : AutoRepathPolicy.Mode.Never; }
        }
        ...
        public Vector3 velocity
        {
            get
            {
                return Time.deltaTime > 0.00001f
                    ? (previousPosition1 - previousPosition2) / Time.deltaTime
                    : Vector3.zero;
            }
        }
        ...
        {
            get { return Mathf.Max(interpolator.remainingDistance, 0); }
        ...
        [SerializeField] [HideInInspector] [UnityEngine.Serialization.FormerlySerializedAs("canSearch")]
        ...
        {
            get { return canSearchAgain && autoRepath.ShouldRecalculatePath((IAstarAI)this); }
        }
        ...
        protected virtual void ConfigureNewPath()
        {
            ...
            if (interpolatePathSwitches && switchPathInterpolationSpeed > 0.01f && hadValidPath)
            {
                var correctionFactor =
                    Mathf.Max(-Vector3.Dot(prevTangent.normalized, interpolator.tangent.normalized), 0);
        ...
        Quaternion SimulateRotationTowards(Vector3 direction, float deltaTime)
        {
            ...
            if (direction != Vector3.zero)
            {
                Quaternion targetRotation = Quaternion.LookRotation(direction,
                    orientation == OrientationMode.YAxisForward ? Vector3.back : Vector3.up);
        ...
        protected virtual Vector3 CalculateNextPosition(out Vector3 direction, float deltaTime)
        {
            ...
            if (interpolatePathSwitches && alpha < 1f)
            {
                ...
                Vector3 positionAlongPreviousPath = previousMovementOrigin +
                                                    Vector3.ClampMagnitude(previousMovementDirection,
                                                        speed * pathSwitchInterpolationTime);
