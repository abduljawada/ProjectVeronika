...
public class AstarPath : VersionedMonoBehaviour
{
    ...
    /// </summary>
    [SerializeField] protected string[] tagNames = null;
    ...
    {
        get { return batchGraphUpdates; }
        set { batchGraphUpdates = value; }
    }
    ...
    {
        get { return graphUpdateBatchingInterval; }
        set { graphUpdateBatchingInterval = value; }
    }
    ...
    /// </summary>
    [System.NonSerialized] public PathHandler debugPathData;
    ...
    /// </summary>
    [System.NonSerialized] bool isScanningBacking;
    ...
    {
        get { return isScanningBacking; }
        private set { isScanningBacking = value; }
    }
    ...
    {
        get { return pathProcessor.NumThreads; }
    }
    ...
    {
        get { return pathProcessor.IsUsingMultithreading; }
    }
    ...
    {
        get { return IsAnyGraphUpdateQueued; }
    }
    ...
    {
        get { return graphUpdates.IsAnyGraphUpdateInProgress; }
    }
    ...
    {
        get { return workItems.workItemsInProgress; }
    }
    ...
    {
        get { return workItems.workItemsInProgressRightNow; }
    }
    ...
    [System.ObsoleteAttribute] public System.Action OnGraphsWillBeUpdated;

    /// <summary>Deprecated:</summary>
    [System.ObsoleteAttribute] public System.Action OnGraphsWillBeUpdated2;
    ...
    void RecalculateDebugLimits()
    {
        ...
        for (int i = 0; i < graphs.Length; i++)
        {
            if (graphs[i] != null && graphs[i].drawGizmos)
            {
                ...
                {
                    if (node.Walkable && (ignoreSearchTree ||
                                          Pathfinding.Util.GraphGizmoHelper.InSearchTree(node, debugPathData,
                                              debugPathID)))
                    {
    ...
    public void UpdateGraphs(GraphUpdateObject ob)
    {
        ...
        {
            throw new System.Exception(
                "You are trying to update graphs using the same graph update object twice. Please create a new GraphUpdateObject instead.");
        }
    ...
    public static int CalculateThreadCount(ThreadCount count)
    {
        ...
        if (count == ThreadCount.AutomaticLowLoad || count == ThreadCount.AutomaticHighLoad)
        {
            ...
            {
                Debug.LogError(
                    "Machine reporting that is has <= 0 bytes of RAM. This is definitely not true, assuming 1 GiB");
                memory = 1024;
            }
    ...
    internal void VerifyIntegrity()
    {
        ...
        {
            throw new System.Exception(
                "Singleton pattern broken. Make sure you only have one AstarPath object in the scene");
        }
    ...
    public IEnumerable<Progress> ScanAsync(NavGraph[] graphsToScan = null)
    {
        ...
        {
            throw new System.Exception(
                "Async scanning can only be done in the pro version of the A* Pathfinding Project");
        }
    ...
    public static void BlockUntilCalculated(Path path)
    {
        ...
        {
            Debug.LogError(
                "You are calling the BlockUntilCalculated function recursively (maybe from a path callback). Please don't do this.");
        }
        ...
        if (path.PipelineState < PathState.ReturnQueue)
        {
            ...
            else
            {
                while (path.PipelineState < PathState.ReturnQueue)
                {
                    ...
                    {
                        waitForPathDepth--;
                        throw new System.Exception("Critical error. Path Queue is empty but the path state is '" +
                                                   path.PipelineState + "'");
                    }
    ...
    /// </summary>
    [System.Obsolete(
        "Use AddWorkItem(System.Action) instead. Note the slight change in behavior (mentioned in the documentation).")]
    ...
    public static void StartPath(Path path, bool pushToFront = false)
    {
        ...
        if (path.PipelineState != PathState.Created)
        {
            throw new System.Exception("The path has an invalid state. Expected " + PathState.Created + " found " +
                                       path.PipelineState + "\n" +
    ...
    public NNInfo GetNearest(Vector3 position, NNConstraint constraint, GraphNode hint)
    {
        ...
        if (!constraint.Suitable(nearestNode.node) || (constraint.constrainDistance &&
                                                       (nearestNode.clampedPosition - position).sqrMagnitude >
