...
namespace Pathfinding
{
    ...
    public class RaycastModifier : MonoModifier
    {
        ...
        [Tooltip(
            "Use raycasting on the graphs. Only currently works with GridGraph and NavmeshGraph and RecastGraph. This is a pro version feature.")]
        ...
        /// </summary>
        [Tooltip(
            "When using the high quality mode the script will try harder to find a shorter path. This is significantly slower than the greedy low quality approach.")]
        ...
        public override void Apply(Path p)
        {
            ...
            if (ValidateLine(null, null, p.vectorPath[0], p.vectorPath[p.vectorPath.Count - 1],
                    cachedFilter.cachedDelegate, cachedNNConstraint))
            {
            ...
            else
            {
                ...
                for (int it = 0; it < iterations; it++)
                {
                    ...
                    points = quality >= Quality.High
                        ? ApplyDP(p, points, cachedFilter.cachedDelegate, cachedNNConstraint)
                        : ApplyGreedy(p, points, cachedFilter.cachedDelegate, cachedNNConstraint);
                }
            ...
            NNConstraint nnConstraint)
        {
            ...
            while (startIndex < points.Count)
            {
                ...
                var startNode = canBeOriginalNodes && points[startIndex] == (Vector3)p.path[startIndex].position
                    ? p.path[startIndex]
                    : null;
                buffer.Add(start);
                ...
                while (true)
                {
                    ...
                    var endNode = canBeOriginalNodes && end == (Vector3)p.path[endIndex].position
                        ? p.path[endIndex]
                        : null;
            ...
            for (int i = 0; i < points.Count; i++)
            {
                ...
                for (int j = i + 1; j < points.Count; j++)
                {
                    ...
                    if (DPParents[j] == -1 || d2 < DPCosts[j])
                    {
                        ...
                        if (j == i + 1 || ValidateLine(startIsOriginalNode ? p.path[i] : null,
                                endIsOriginalNode ? p.path[j] : null, start, points[j], filter, nnConstraint))
                        {
                            DPCosts[j] = d2;
                            DPParents[j] = i;
                        }
        ...
        protected bool ValidateLine(GraphNode n1, GraphNode n2, Vector3 v1, Vector3 v2,
            System.Func<GraphNode, bool> filter, NNConstraint nnConstraint)
        {
            if (useRaycasting)
            {
                ...
                else
                {
                    ...
                    if (thickRaycast && thickRaycastRadius > 0)
                    {
                        ...
                        if (Physics.CheckSphere(v1 + raycastOffset + (v2 - v1).normalized * thickRaycastRadius,
                                thickRaycastRadius, mask))
                        {
                            return false;
                        }

                        if (Physics.SphereCast(new Ray(v1 + raycastOffset, v2 - v1), thickRaycastRadius,
                                (v2 - v1).magnitude, mask))
                        {
                            return false;
                        }
                    }
